#include "passager.h"
#include "tri.h" // Récupération du tri sélection décroissant
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h> // Création d'une structure qui va nous servir à récupérer la date du jour
...
// Cette fonction renvoi le nombre de jours qui sépare les 2 jours d'une date en fonction du mois et de l'année
...
void afficherPassager(Vol *vols, int taille, const char *nomFichier) {
    ...
    int annee = timeinfo->tm_year + 1900;   // Le compteur d'année commence à l'année 1900
    int mois = timeinfo->tm_mon + 1;    // Le compteur du mois commence à 0 pour le mois janvier
    int jour = timeinfo->tm_mday;

    for (i = 1; i != n_vol; i++) { // Recherche du numéro de vol
    }
    i -= 1;
    ...
    for (j = 0; j < cpt; j++) {
        sscanf(vols[i].passager[j].date_naiss, "%d/%d/%d", &day, &month, &year);    // Récupération de la date sous format DD/MM/YYYY
        ...
        somme_total = cpt_day + cpt_month + cpt_year;       // Calcul du nombre total de jours séparant la date du jour et celle du passager
        if (somme_total < 4383){
    ...
    for(j=0; j<cpt; j++){
        for (k=j; k<cpt; k++){
            if ( vols[i].passager[k].prix_billet == tab[j] && n < cpt2 && unique1(indice, n, k) == 1 ){     // Récupéraion des indices des passagers qui correspondent au tableau trié
                indice[n]= k;
                n += 1;
            }
        }
    ...
    for(j=0; j<cpt; j++){
        for (k=0; k<cpt; k++){
            if ( vols[i].passager[k].prix_billet == tab2[j] && n < cpt3 &&  unique1(indice2, n, k) == 1){   // Récupéraion des indices des passagers qui correspondent au tableau trié
                indice2[n]= k;;
                n += 1;
            }
        }
    ...
    /*strcmp permet la comparaison de 2 chaines de caractères,
    elle renvoi 1 si la 1ere est après dans l'ordre alphabétique, 0 si elles sont identiques et -1 si la 1ere la 1ere est a dans l'ordre alphabétique */


    for (j=0;j < cpt2-1; j++) {
        if (vols[i].passager[indice[j]].prix_billet == vols[i].passager[indice[j+1]].prix_billet) {     // Tri en fonction des noms si les prix des billets sont égaux
    ...
    for (j=0;j < cpt3-1; j++) {
        if (vols[i].passager[indice2[j]].prix_billet == vols[i].passager[indice2[j+1]].prix_billet) {   // Tri en fonction des noms si les prix des billets sont égaux
    ...
    for (j=0;j < cpt2-1; j++) {
        if (strcmp(vols[i].passager[indice[j]].nom, vols[i].passager[indice[j+1]].nom)== 0 && vols[i].passager[indice[j]].prix_billet == vols[i].passager[indice[j+1]].prix_billet) {
           if (strcmp(vols[i].passager[indice[j]].prenom, vols[i].passager[indice[j+1]].prenom) > 0) {  // Tri en fonction des prenoms si les noms sont les mêmes
                temp2 = indice[j];
                indice[j] = indice[j+1];
                indice[j+1] = temp2;
            }
    ...
    for (j=0;j < cpt3-1; j++) {
        if (strcmp(vols[i].passager[indice2[j]].nom, vols[i].passager[indice2[j+1]].nom)== 0 && vols[i].passager[indice2[j]].prix_billet == vols[i].passager[indice2[j+1]].prix_billet) {
           if (strcmp(vols[i].passager[indice2[j]].prenom, vols[i].passager[indice2[j+1]].prenom) > 0) {    // Tri en fonction des prenoms si les noms sont les mêmes
                temp2 = indice2[j];
                indice2[j] = indice2[j+1];
                indice2[j+1] = temp2;
            }
    ...
    k=0;
    for (j=0;j < cpt; j++ ){
        if (strcmp(vols[i].passager[j].nom, nom) == 0){     // Recherche du nom rentré
    ...
    printf("\n");
    if (trouver == 0){ // Cas où le nom n'est pas répertorié dans la base de donnée
        printf("Le nom '%s' est introuvable pour ce vol.\n\n", nom);
    }
    else if (trouver > 1){ // Cas où il y aurait plusieurs personnes avec le même nom
        printf("Entrer un Prenom : ");
        scanf("%s",&prenom);
        printf("\n");
        for (j=0;j < k; j++ ){
            if (strcmp(vols[i].passager[resultat[j]].prenom, prenom) == 0){     // Recherche du prénom rentré
        ...
        printf("\n");
        if (trouver2 == 0){     // Cas où le prénom n'est pas répertorié dans la base de donnée
